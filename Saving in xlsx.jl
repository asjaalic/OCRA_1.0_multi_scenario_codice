...
function saving_OCRA_2(InputParameters::InputParam, Results_OCRA_2::Results_OCRA2, Results_ex_post::results_ex_post, Results_statistics_OCRA_2::Data_analysed, Battery::BatteryParam,  NScen)
    ...
    for iScen=1:NScen
            ...
            stage_results[!, "Revamping costs €"] = cost_rev[iScen,:]
            stage_results[!, "Net revenues €"] = net_revenues_per_stage[iScen,:]
            stage_results[!, "Binary revamping"] = e[iScen,:]
            stage_results[!, "Rev Acquisto €"] = rev_acquisto[iScen,:]
            stage_results[!, "Rev Vendita €"] = rev_vendita[iScen,:]

            hourly_results = DataFrame()

            hourly_results[!, "Steps"] = 1:1:NSteps_scenario[iScen]
            hourly_results[!, "Energy prices €/MWh"] = vector_prices[iScen][:]
            hourly_results[!, "Capacity MWh"] = cap[iScen][1:end-1]
            hourly_results[!, "SOC Mwh"] = soc[iScen][1:end-1]
            hourly_results[!, "SOC_quad Mwh"] = soc_quad[iScen][1:end-1]
            hourly_results[!, "Charge MWh"] = charge[iScen][:]
