...
function ex_post_analysis(Results_No_Deg::ResultWithoutDeg, InputParameters::InputParam, Battery::BatteryParam, NScen, NSteps, Pp, Steps_stages)

    @unpack (charge_no_deg, discharge_no_deg, soc_no_deg, soc_quad_no_deg, revenues_per_stage_no_deg) = Results_No_Deg
    @unpack (NYears, NMonths, NHoursStep, NStages, Big, conv, disc) = InputParameters
...
function statistical_analysis(ResultsOpt::Results, Results_ex_post::results_ex_post, NScen, NStages)

    @unpack (net_revenues_per_stage, WEM_stage, cost_rev, deg_stage, rev, cap) = ResultsOpt;
